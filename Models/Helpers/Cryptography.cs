    ...
    public class Cryptography
    {
        public static string Encrypt(string encryptString, string encryptionKey="0ram@1234xxxxxxxxxxtttttuuuuuiiiiio")
        {
            if (string.IsNullOrEmpty(encryptString) || string.IsNullOrEmpty(encryptionKey))
                return "";
            //string EncryptionKey = "0ram@1234xxxxxxxxxxtttttuuuuuiiiiio"; //itshould be random sir. //we can change the code converstion key as per our requirement, but the decryption key should be same as encryption key    
